...
class MazeWithRooms(Dungeon):
    ...
    def generate_level(self, map_width, map_height):
        ...
        self.addRooms(map_width, map_height)  # ?
        ...
        for y in range(1, map_height, 2):
            for x in range(1, map_width, 2):
                if self.level[x][y] != 1:
                    continue
                start = (x, y)
                self.growMaze(start, map_width, map_height)

        self.connectRegions(map_width, map_height)

        if not self.allowDeadEnds:
            self.removeDeadEnds(map_width, map_height)

        return self.level

    def growMaze(self, start, map_width, map_height):
        north = (0, -1)
        south = (0, 1)
        east = (1, 0)
        west = (-1, 0)
        ...
        while cells:
            ...
            for direction in [north, south, east, west]:
                if self.canCarve(cell, direction, map_width, map_height):
    ...
    def addRooms(self, map_width, map_height):
        rooms = []
    ...
    def removeDeadEnds(self, map_width, map_height):
        done = False
    ...
    def canCarve(self, pos, dir, map_width, map_height):
        '''
