...
def draw(entity, game_map, x, y):
    ...
    if game_map.name is "hub" and entity.player:
        blt.put(x * options.tile_offset_x, y *
                options.tile_offset_y, entity.char_hub)
    else:
        blt.put(x * options.tile_offset_x, y *
                options.tile_offset_y, entity.char)
...
def draw_map(game_map, game_camera, fov_map, fov_recompute, viewport_x, viewport_y):
    ...
    if fov_recompute:
        ...
        for y in range(ceil(options.camera_offset), game_camera.height - ceil(options.camera_offset)):
            for x in range(ceil(options.camera_offset), game_camera.width - ceil(options.camera_offset)):
                ...
                if visible:
                    ...
                    if not game_map.tiles[map_x][map_y].char == " ":
                        ...
                        blt.put(x * options.tile_offset_x, y * options.tile_offset_y,
...
def draw_messages(msg_panel, message_log, player, power_msg, viewport_x, viewport_y):
    ...
    blt.puts(3, viewport_y + options.ui_offset_y + 1, "[offset=0,-2]" + hp)
    blt.color("default")
    blt.puts(5 + len(hp), viewport_y +
             options.ui_offset_y + 1, "[offset=0,-2]" + ac)
    blt.puts(7 + len(hp) + len(ac), viewport_y +
             options.ui_offset_y + 1, "[offset=0,-2]" + ev)
    blt.puts(9 + len(hp) + len(ac) + len(ev),
             viewport_y + options.ui_offset_y + 1, "[offset=0,-2]" + power)

    if message_log.update:
        blt.layer(1)
        blt.clear_area(msg_panel.x * options.ui_offset_x, msg_panel.y * options.ui_offset_y, msg_panel.w *
                       options.ui_offset_x, msg_panel.h * options.ui_offset_y)
        blt.color("white")
        ...
        for msg in message_log.buffer:
            msg = shorten(msg, msg_panel.w * options.ui_offset_x - 2,
                          placeholder="..(Press 'M' for log)")
            blt.puts(msg_panel.x * options.ui_offset_x + 1, msg_panel.y *
                     options.ui_offset_y + i, "[offset=0,9]" + msg, msg_panel.w * options.ui_offset_x - 2, 1, align=blt.TK_ALIGN_LEFT)
            i += 1
    ...
    for y in range(msg_panel.y, msg_panel.h):
        for x in range(msg_panel.x, msg_panel.w):
            blt.put_ext(x * options.ui_offset_x, y *
                        options.ui_offset_y, 0, 0, 0xE900 + 19)
    ...
    for y in range(msg_panel_borders.y, msg_panel_borders.h):
        for x in range(msg_panel_borders.x, msg_panel_borders.w):
            if (y == msg_panel_borders.y):
                blt.put_ext(x * options.ui_offset_x, y *
                            options.ui_offset_y, 0, 10, 0xE900 + 472)
            elif (y == msg_panel_borders.h - 1):
                blt.put_ext(x * options.ui_offset_x, y *
                            options.ui_offset_y, 0, -10, 0xE900 + 441)
            elif(x == msg_panel_borders.x):
                blt.put_ext(x * options.ui_offset_x, y *
                            options.ui_offset_y, 10, 0, 0xE900 + 440)
            elif(x == msg_panel_borders.w - 1):
                blt.put_ext(x * options.ui_offset_x, y *
                            options.ui_offset_y, -10, 0, 0xE900 + 476)
    ...
    for y in range(screen_borders.y, screen_borders.h):
        for x in range(screen_borders.x, screen_borders.w):
            if (y == screen_borders.y):
                blt.put_ext(x * options.ui_offset_x, y *
                            options.ui_offset_y, 0, 0, 0xE900 + 472)
            elif (y == screen_borders.h - 1):
                blt.put(x * options.ui_offset_x, y *
                        options.ui_offset_y, 0xE900 + 441)
            elif(x == screen_borders.x):
                blt.put(x * options.ui_offset_x, y *
                        options.ui_offset_y, 0xE900 + 440)
            elif(x == screen_borders.w - 1):
                blt.put(x * options.ui_offset_x, y *
                        options.ui_offset_y, 0xE900 + 476)
...
def clear(entity, x, y):
    ...
    blt.layer(entity.layer)
    blt.clear_area(x * options.tile_offset_x, y * options.tile_offset_y, 1, 1)
