...
def draw_entities(entities, player, game_map, game_camera, fov_map):
    ...
    for category in entities.values():
        for entity in category:
            ...
            if (0 < x < game_camera.width - ceil(
                    variables.camera_offset) and 0 < y < game_camera.height - ceil(variables.camera_offset)):
                ...
                if "cursor" in entities.keys():
                    if (entity.x == entities["cursor"][0].x and
                            entity.y == entities["cursor"][0].y and not
                        ...
                        variables.stack.append(str("x: "+(str(entities["cursor"][0].x) +", y: " + str(entities["cursor"][0].y))))
...
def draw_messages(msg_panel, message_log):
    ...
    if message_log.new_msgs:
        blt.layer(1)
        blt.clear_area(msg_panel.x * variables.tile_offset_x, msg_panel.y * variables.tile_offset_y, msg_panel.w *
                       variables.tile_offset_x, msg_panel.h * variables.tile_offset_y)
        blt.color("white")
...
def draw_stats(player, target=None):
    ...
    blt.layer(1)
    blt.clear_area(2, variables.viewport_y + variables.tile_offset_y + 1,
    ...
    if variables.viewport_x > 90:
        ...
        #                  variables.viewport_y + variables.tile_offset_y, "[offset=0,5]" + "[U+EAB8]", 0, 0, blt.TK_ALIGN_CENTER)
        #         blt.puts(int(variables.viewport_x / 2) + int(len(power_msg) / 2 + 3),
        #                  variables.viewport_y + variables.tile_offset_y, "[offset=0,2]" + "[U+EADC]", 0, 0, blt.TK_ALIGN_CENTER)
        blt.color("default")
        blt.puts(int(variables.viewport_x / 2),
                 variables.viewport_y + variables.tile_offset_y + 1, "[offset=0,-2]" + power_msg, 0, 0,
                 blt.TK_ALIGN_CENTER)
    else:
    ...
    blt.puts(4, variables.viewport_y + variables.tile_offset_y + 1,
    ...
    if target:
        blt.clear_area(int(variables.viewport_x / 2) - int(len(power_msg) / 2) - 5,
                       variables.viewport_y + variables.tile_offset_y + 1, variables.viewport_x, 1)
        ...
        blt.puts(variables.viewport_x, variables.viewport_y + variables.tile_offset_y + 1,
...
def draw_all(game_map, game_camera, entities, player, fov_map,
             fov_recompute, msg_panel, msg_panel_borders, screen_borders):
