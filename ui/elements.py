...
def init_ui():
    ...
    w = floor(screen_w / variables.tile_offset_x)
    h = floor(screen_h / variables.tile_offset_y - 5)

    msg_panel = Panel(1, h + 1, w - 1, h + 4)
    msg_panel_borders = Panel(0, h, w, h + 5)
    screen_borders = Panel(0, 0, w, h)

    viewport_x = w * variables.tile_offset_x - (variables.tile_offset_x + 1)
    viewport_y = h * variables.tile_offset_y - (variables.tile_offset_y + 1)
